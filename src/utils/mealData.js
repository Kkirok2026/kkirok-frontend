export const MEAL_LABELS = {
  ...
  export function toMealDisplay(mealLog, fallbackKey) {
    ...
    const visibleItems = (mealLog?.items || [])
      .filter((item) => !item.excluded)
      .map((item) => ({
        ...item,
        mealLogId: item.mealLogId || mealLog?.mealLogId,
      }));
  
    return {
      mealKey,
      mealLogId: mealLog?.mealLogId,
      mealLogIds: mealLog?.mealLogId ? [mealLog.mealLogId] : [],
  ...
  function mergeMealDisplay(previous, next) {
    if (!hasMealData(previous)) return next;
    if (!hasMealData(next)) return previous;
  
    const items = [...(previous.items || []), ...(next.items || [])];
  
    const mealLogIds = Array.from(
      new Set(
        [
          ...(previous.mealLogIds || []),
          ...(next.mealLogIds || []),
    ...
    return {
      ...previous,
      ...next,
      mealLogId: next.mealLogId || previous.mealLogId,
      mealLogIds,
      kcal: toRounded((previous.kcal || 0) + (next.kcal || 0)),
      carbs: toRounded((previous.carbs || 0) + (next.carbs || 0)),
      protein: toRounded((previous.protein || 0) + (next.protein || 0)),
      fat: toRounded((previous.fat || 0) + (next.fat || 0)),
      sugar: toRounded((previous.sugar || 0) + (next.sugar || 0)),
      sodium: toRounded((previous.sodium || 0) + (next.sodium || 0)),
      items,
      foods: items.map((item) => item.itemName),
    };
  }
  ...
  export function toMealsByType(mealLogList) {
    ...
    for (const mealLog of getMealLogItems(mealLogList)) {
      const meal = toMealDisplay(mealLog);
      meals[meal.mealKey] = mergeMealDisplay(meals[meal.mealKey], meal);
    }
